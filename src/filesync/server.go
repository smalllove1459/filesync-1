...
func (myRequest *MyRequestToRemote) getDownloadProgressString () string {
	...
	if myRequest.fileSize <= 0 {
		percentage = "0% downloaded"
	} else {
		percentage = "downloading" // todo: strconv.FormatFloat(float64 (myRequest.downloaded) * 100.0 / float64 (myRequest.fileSize), 'f', 2, 32) + "%"
	}
...
func (myRequest *MyRequestToRemote) error(msg string, fatal bool) {
	log.Printf("Error on file (%s) syncing: %s\n", myRequest.filePath, msg)
	if fatal {
...
func (myRequest *MyRequestToRemote) ok(numNewBytes int64, justFinished bool, utcTime time.Time) {
	if justFinished {
		...
		log.Printf("Finished file (%s) syncing.\n", myRequest.filePath)
	} else {
		log.Printf("Stopped file (%s) syncing.\n", myRequest.filePath)
	}
...
func (myRequest *MyRequestToRemote) run() {
	...
	if response.StatusCode == http.StatusNotModified { // 304
		...
		myRequest.ok (num_bytes, num_bytes == int64 (content_length) && new_size == target_file_info.fileSyncingInfo.ContentLength, target_file_info.fileSyncingInfo.LastModified)
		...
		myRequest.ok (num_bytes, num_bytes == int64 (content_length), last_modified)
